...
class Bot:
    ...
    def __init__(
        ...
        self.candles = self.client.public.get_candles(
            market=MARKET_ETH_USD, resolution=CANDLE_RESOLUTION, limit=100
        )
        self.data = self.candles.data["candles"]
    ...
    def get_account_balance_usd(self):
        ...
        return self.account["balance"]["usd"]

    def load_all_histories(self):
        with open(f"{self.histories_fname}.json", "r") as f:
    ...
    def run_meanreversion_strategy(self):
        for market in [f"{b}-{QUOTATION_ASSET}" for b in BASE_ASSETS]:
            ...
            buy_order = buy_orders["orders"][0] if buy_orders["orders"] else None
            ...
            sell_order = sell_orders["orders"][0] if sell_orders["orders"] else None
            ...
            if not self.positions["long"]:
                ...
                if self.get_entry_signal(float(price)) and not buy_order:
                    ...
                    size = str(max(size, float(self.market_info["minOrderSize"])))
                    order_params = {
                ...
                if self.get_stop_signal(entry_price, float(price)):
                    if buy_order:
                        self.client.private.cancel_order(order_id=buy_order["id"])
                    if sell_order:
                        self.client.private.cancel_order(order_id=sell_order["id"])
                    order_params = {
                        "position_id": self.account["positionId"],
                        "market": market,
                        "side": ORDER_SIDE_SELL,
...
bot = Bot()
#account_balance_usd = bot.get_account_balance_usd()
#print(account_balance_usd)
